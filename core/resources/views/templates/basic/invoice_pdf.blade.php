  ...
  <div class="container" style="margin:0 5%;">
    <table style="padding-top:5%;">
      <tbody>
        <tr>
          ...
          <td align="right">
            <p>Invoice NO. : {{__($invoice->prefix)}}-00{{__($invoice->id)}}</p>
    ...
    <table>
      <tbody>
        <tr>
          <td  width="66%">
            <h4>{{$general->cur_sym}} {{getAmount($invoice->invoice_amount)}} Agency Fee</h4>
            <p>{{$job_details[0]->job->title}} - {{showDateTime($invoice->created_at, 'M d,Y')}}</p>
    ...
    <table style="border-bottom: 1px solid transparent;">
      <tbody>
        <tr>
          <td>
            <h4 style="line-height:5px;">Subtotal</h4>
            <p style="line-height:5px;">TAX({{$invoice->tax_rate}}%)</p>
            <h4 style="line-height:5px;">Total </h4>
          </td>
          <td align="right">
            <h4 style="line-height:5px;">{{$general->cur_sym}} {{getAmount($invoice->invoice_amount)}}</h4>
            <h4 style="text-decoration:underline;line-height:5px;">{{$general->cur_sym}} {{getAmount($invoice->tax_amt)}}</h4>
            <h4 style="line-height:5px;">{{$general->cur_sym}} {{getAmount($invoice->invoice_amt_with_tax)}}</h4>
          </td>
        </tr>
      </tbody>
    </table>
