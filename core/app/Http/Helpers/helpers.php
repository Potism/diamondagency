...
function getIpInfo()
{
    ...
    $LocationArray = json_decode(curlPostContent('http://ip-get-geolocation.com/api/json/'.$ip,[]));
    if($LocationArray->status != 'fail')
    {
        $data['country'] = $LocationArray->country;
        $data['city'] = $LocationArray->city;
        $data['area'] = $LocationArray->regionName;
        $data['code'] = $LocationArray->countryCode;
        $data['long'] = $LocationArray->lon;
...
function sendSms($user, $type, $shortCodes = [])
{
    ...
    if ($general->sn == 1 && $smsTemplate) {
        ...
        if($user->mobile)
        {
            $sendSms->$gateway($user->mobile_code.$user->mobile,$general->sitename,$message,$general->sms_config);
        }
    }
}
...
{
    $data['employerLogo'] = [
        'path' => 'assets/images/employerLogo',
    ];
    $data['gateway'] = [
...
function gatewayRedirectUrl($type = false){
    if ($type) {
        return 'employer.deposit.history';
    }else{
        return 'employer.deposit';
    }
}
...
function  get_rating($val)
{
    ...
    if($val > 0)
    {
        ...
        for($i = 0;$i < $num;$i++)
        {
            $html.='<i style="color:#ffd700 !important;" class="fas fa-star"></i>';
        }
        if($diff > 0 && $diff<= 0.5)
        {
            $html.='<i style="color:#ffd700 !important;" class="fa fa-star-half-o"></i>';
            $num = $num+1;
        }
        elseif($diff > 0 && $diff<= 0.5)
        {
            $html.='<i style="color:#ffd700 !important;" class="fa fa-star"></i>';
            $num = $num+1;
        }
        for($j = $num;$j < 5;$j++)
        {
            $html.='<i style="color:#ffd700 !important;" class="fa fa-star-o"></i>';
        }
    }
    else
    {
...
function distance($latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo, $unit = 'kilometers', $earthRadius = 6371000)
{
...
{
    $user = Auth::user();
    if($user->firstname != '' && $user->lastname != '' && $user->email != '' && $user->country_code != '' && $user->mobile != '' && $user->address->address != '' && $user->address->state != '' && $user->address->zip != '' && $user->address->country != '' && $user->address->city != '' && $user->designation != '' && $user->language != ''  && $user->software_id != '' && $user->national_id != '' && $user->candidate_rate != '' && $user->birth_date != '')
    {
        return true;
    }
    else
...
function auto_logout()
{
    $t = time();
    $t0 = Session::get('sess');
    if(auth()->guard('employer')->user() || auth()->user() || auth()->guard('admin')->user())
    {
        if(!isset($t0))
        {
            Session::put('sess', time());
        }
        else
        {
            $diff = $t - $t0;
            ...
            {     
                ...
                if(auth()->user())
                {
                    Session::put('sess',null);
                    Session::flush();
                    return redirect()->route('user.logout')->send();
                }
                if(auth()->guard('employer')->user())
                {
                    Session::put('sess',null);
                    Session::flush();
                    return redirect()->route('employer.logout')->send();
                }
            }
            else
            {
