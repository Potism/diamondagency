...
class PaymentController extends Controller
{
    ...
    public function deposit()
    {
        ...
        return view($this->activeTemplate . 'employer.payment.deposit', compact('gatewayCurrency', 'pageTitle'));
    }
    ...
    public function depositPreview()
    {
        ...
        return view($this->activeTemplate . 'employer.payment.preview', compact('data', 'pageTitle'));
    }
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            $user = Employer::find($data->employer_id);
            $user->balance += $data->amount;
            $user->save();

            $transaction = new Transaction();
            $transaction->employer_id = $data->employer_id;
        ...
        if($data->order_id){
            ...
            if($booking){
                ...
                $transaction->post_balance = $user->balance;
                $transaction->trx_type = '+';
                $transaction->details = 'Plan amount deposited Via ' . $data->gatewayCurrency()->name;
                $transaction->trx = $data->trx;
                $transaction->save();

                $adminNotification = new AdminNotification();
                $adminNotification->user_id = $user->id;
                $adminNotification->title = 'Plan Purchase Complate '.$user->username;
                $adminNotification->click_url = urlPath('admin.employers.detail', $user->id);
                $adminNotification->save();
            }
        }
    }
    ...
    public function manualDepositConfirm()
    {
        ...
        if ($data->method_code > 999) {
            ...
            return view($this->activeTemplate . 'employer.manual_payment.manual_confirm', compact('data', 'pageTitle', 'method'));
        }
        abort(404);
    }
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        return redirect()->route('employer.deposit.history')->withNotify($notify);
    }
