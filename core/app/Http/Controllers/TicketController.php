...
class TicketController extends Controller
{
    ...
    public function supportTicket()
    {
        ...
        else if(Auth::guard('employer')->user()){
            $supports = SupportTicket::where('employer_id', Auth::guard('employer')->user()->id)->orderBy('priority', 'desc')->orderBy('id','desc')->paginate(getPaginate());
            $view = 'employer';
        }
        else{
            abort(404);
        ...
        return view($this->activeTemplate . $view . '.support.index', compact('supports', 'pageTitle'));
    }
    ...
    public function openSupportTicket()
    {
        ...
        return view($this->activeTemplate.$view.'.support.create', compact('pageTitle', 'user'));
    }
    ...
    {   
        ...
        if (Auth::user()) {
            ...
            $my_ticket = SupportTicket::where('ticket', $ticket)->where('employer_id',$employerId)->orderBy('id','desc')->firstOrFail();
        }
          else {
          $my_ticket = SupportTicket::where('ticket', $ticket)->where('user_id',$userId)->orderBy('id','desc')->firstOrFail();
        }
        ...
        if(Auth::user()){
            return view($this->activeTemplate. 'user.support.view', compact('my_ticket', 'messages', 'pageTitle'));
        }elseif(Auth::guard('employer')->user()){
            return view($this->activeTemplate. 'employer.support.view', compact('my_ticket', 'messages', 'pageTitle'));
        }else{
