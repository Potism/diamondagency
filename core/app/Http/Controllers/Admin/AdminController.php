...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $employer['total_employer'] = Employer::count();
        $employer['verified_employer'] = Employer::where('status', 1)->count();

        //Other
        $other['job'] = Job::count();
        $other['job_pending'] = Job::where('status', 0)->count();
        $other['job_running'] = Job::where('status', 1)->whereDate('deadline','>', Carbon::now()->toDateTimeString())->count();
        $other['category'] = Category::count();
        $other['order'] = Order::where('status', '!=', 0 )->count();
        $other['plan'] = Plan::count();
        ...
        $invoices = Invoice::select('*')->latest()->with('user','job','job.employer')->count();
        $low_balance = Employer::orderBy('id','desc')->where('balance','<','200')->count();
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'report', 'chart','payment','depositsMonth','months','deposits', 'employer', 'other','invoices','low_balance'));
    }
